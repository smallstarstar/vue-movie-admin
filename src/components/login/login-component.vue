<template>
  <div class="loginContent">
    <div class="login-form">
      ...
      <!-- <el-button v-if="showClient" class="buttonSub" type="success">注 册</el-button> -->
      <Register v-if="showClient"/>
...
export default class LoginComponent extends Vue {
  ...
  async loginForm() {
    // 将数据存入store中
    let userInfo: any = {};
    const data: any = await userLoginServices.userLoginSystem(this.userInfo);
    // 登录成功
    if (data.messageInfo.code === LoginCode.LOGINSUCCESSCODE) {
      if (this.remember) {
        // 将信息存入localStorage
        const remember: any = {};
      ...
      this.setUserInfo(data.messageInfo.data);
      localStorage.setItem("userInfo", JSON.stringify(data.messageInfo.data));
    ...
    if (data.messageInfo.code === LoginCode.LOGINFAILCODE) {
